...
impl SMPContext {
    ...
    pub fn initiate(&mut self, ssid: &[u8; 8], question: &[u8], secret: &[u8]) -> Result<TLV, OTRError> {
        ...
        let c2 = BigUint::from_bytes_be(&SHA256::digest_with_prefix(1,
            &OTREncoder::new().write_mpi(&g1.modpow(&r2, &MODULUS)).to_vec()));
        let D2: BigUint = (&r2 - &a2*&c2).mod_floor(&Q);
        let c3 = BigUint::from_bytes_be(&SHA256::digest_with_prefix(2,
    ...
    pub fn handleMessage1(&mut self, tlv: TLV, ssid: &SSID, question: &[u8], secret: &[u8]) -> Result<TLV, OTRError> {
        if tlv.0 != TLV_TYPE_SMP_MESSAGE_1 && tlv.0 != TLV_TYPE_SMP_MESSAGE_1Q {
            panic!("BUG: Only TLV's SMP Message 1 and 1Q should be routed here.")
        }
        if let SMPState::Expect1 = &self.smp {
        ...
        let expected_c2 = BigUint::from_bytes_be(&SHA256::digest_with_prefix(1,
        ...
        let expected_c3 = BigUint::from_bytes_be(&SHA256::digest_with_prefix(2,
            &OTREncoder::new().write_mpi(
        ...
        // `D6 = r6 - y cP mod q`."
        let cp = BigUint::from_bytes_be(&SHA256::digest_2_with_prefix(5,
            &encodeBigUint(&g3.modpow(&r5, &MODULUS)),
            &encodeBigUint(&(g1.modpow(&r5, &MODULUS) * g2.modpow(&r6, &MODULUS)).mod_floor(&MODULUS))));
        let D5 = (&r5 - &r4 * &cp).mod_floor(&Q);
        let D6 = (&r6 - &y * &cp).mod_floor(&Q);

        let payload = OTREncoder::new().write_mpi_sequence(&[
    ...
    pub fn handleMessage2(&mut self, tlv: TLV) -> Result<TLV, OTRError> {
        if tlv.0 != TLV_TYPE_SMP_MESSAGE_2 {
            panic!("BUG: Only TLV SMP Message 2 should be routed here.")
        }
        ...
        let a2: &BigUint;
        let a3: &BigUint;
        if let SMPState::Expect2{x: x_, a2: a2_, a3: a3_} = &self.smp {
            x = x_;
            a2 = a2_;
            a3 = a3_;
        } else {
        ...
        let c2_expected = BigUint::from_bytes_be(&SHA256::digest_2_with_prefix(3,
            &OTREncoder::new().write_mpi(&g1.modpow(&D2, &MODULUS)).to_vec(),
            &OTREncoder::new().write_mpi(&g2b.modpow(&c2, &MODULUS)).to_vec()));
        DH::verify(&c2_expected, &c2).or_else(|err| Err(OTRError::CryptographicViolation(err)))?;

        // "Check that `c3 = SHA256(4, g1D3 g3bc3)`."
        let c3_expected = BigUint::from_bytes_be(&SHA256::digest_2_with_prefix(4,
            &OTREncoder::new().write_mpi(&g1.modpow(&D3, &MODULUS)).to_vec(),
            &OTREncoder::new().write_mpi(&g3b.modpow(&c3, &MODULUS)).to_vec()));
        ...
        let cp_expected = BigUint::from_bytes_be(&SHA256::digest_2_with_prefix(5,
            &OTREncoder::new().write_mpi(&(&g3.modpow(&D5, &MODULUS) * &pb.modpow(&cp, &MODULUS)).mod_floor(&MODULUS)).to_vec(),
            &OTREncoder::new().write_mpi(&(&g1.modpow(&D5, &MODULUS) * &g2.modpow(&D6, &MODULUS) * &qb.modpow(&cp, &MODULUS)).mod_floor(&MODULUS)).to_vec()));
        ...
        let cp = BigUint::from_bytes_be(&SHA256::digest_2_with_prefix(6,
            &OTREncoder::new().write_mpi(&g3.modpow(&r5, &MODULUS)).to_vec(),
            &OTREncoder::new().write_mpi(&(&g1.modpow(&r5, &MODULUS) * &g2.modpow(&r6, &MODULUS).mod_floor(&MODULUS)))
            .to_vec()));
        ...
        let tlv = TLV(TLV_TYPE_SMP_MESSAGE_3, OTREncoder::new().write_mpi_sequence(&[

        ]).to_vec());
        // "Set smpstate to SMPSTATE_EXPECT4."
        self.smp = SMPState::Expect4{a3, g3b, padivb, qadivb};
        Ok(tlv)
    }

    pub fn handleMessage3(&mut self, tlv: TLV) -> Result<TLV, OTRError> {
        if tlv.0 != TLV_TYPE_SMP_MESSAGE_3 {
            panic!("BUG: Only TLV SMP Message 3 should be routed here.")
        }
        let g3a: &BigUint;
        let g2: &BigUint;
        let g3: &BigUint;
        ...
        let pb: &BigUint;
        let qb: &BigUint;
        if let SMPState::Expect3{g3a: g3a_, g2: g2_, g3: g3_, b3: b3_, pb: pb_, qb: qb_} = &self.smp {
            g3a = g3a_;
            g2 = g2_;
            g3 = g3_;
            b3 = b3_;
        ...
        If smpstate is SMPSTATE_EXPECT3:
            Verify Alice's zero-knowledge proofs for Pa, Qa and Ra:
                ...
                Check that cP = SHA256(6, g3D5 PacP, g1D5 g2D6 QacP).
    ...
    pub fn handleMessage4(&mut self, tlv: TLV) -> Result<TLV, OTRError> {
        if tlv.0 != TLV_TYPE_SMP_MESSAGE_4 {
            panic!("BUG: illegal TLV type provided at method call.")
        }
        let g3b: &BigUint;
        let padivb: &BigUint;
        let qadivb: &BigUint;
        let a3: &BigUint;
        if let SMPState::Expect4{g3b: g3b_,padivb: padivb_, qadivb: qadivb_, a3: a3_} = &self.smp {
            g3b = g3b_;
            padivb = padivb_;
            qadivb = qadivb_;
            a3 = a3_;
        ...
        let expected_cR = BigUint::from_bytes_be(&SHA256::digest_2_with_prefix(8u8,
            &OTREncoder::new().write_mpi(&(g1.modpow(&D7, &MODULUS) * g3b.modpow(&cR, &MODULUS))).to_vec(),
            &OTREncoder::new().write_mpi(
                &((&Qa/&Qb).modpow(&D7, &MODULUS)*(&Rb.modpow(&cR, &MODULUS))).mod_floor(&MODULUS)
            ).to_vec()
        ));
